...
$('#loginForm').addEventListener('submit',async e=>{
  e.preventDefault(); const submit=e.submitter; submit.disabled=true; submit.textContent='CONNECTING…';
...
async function getJson(url){const r=await fetch(proxy(url));if(!r.ok)throw new Error(`Provider returned ${r.status}`);return r.json()}
async function getText(url){const r=await fetch(proxy(url));if(!r.ok)throw new Error(`Provider returned ${r.status}`);return r.text()}
...
async function connectXtream(){
  ...
  const [live,movies,series]=await Promise.all([getJson(apiUrl('get_live_streams')),getJson(apiUrl('get_vod_streams')),getJson(apiUrl('get_series'))]);
...
function normalizeXtream(x,type){
  ...
  return{id:String(id),name:x.name||x.title||'Untitled',type,category:String(x.category_id||''),icon:x.stream_icon||x.cover||'',url,catchup:String(x.tv_archive)==='1',rating:x.rating_5based||x.rating||'',raw:x};
}
...
function isRestricted(x){return /adult|xxx|18\+/i.test(String(x.category||''))}
...
function play(item){
  if(item.type==='series'){toast('Episode browser will open after server connection is finalized');return}
...
$('#pinForm').onsubmit=e=>{e.preventDefault();if($('#pinInput').value===localStorage.getItem('tt-pin')){state.unlocked=true;$('#pinDialog').close();toast('Parental controls unlocked');render()}else toast('Incorrect PIN')};

window.addEventListener('keydown',e=>{if(!['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key))return;const focusables=[...document.querySelectorAll('button:not([disabled]),input,select')].filter(x=>x.offsetParent);const current=document.activeElement,i=Math.max(0,focusables.indexOf(current));focusables[Math.min(focusables.length-1,i+(e.key==='ArrowRight'||e.key==='ArrowDown'?1:-1))]?.focus()});
